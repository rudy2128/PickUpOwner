...
class TrxAdapter(private var trxList: List<Trx>,private var code:String):
RecyclerView.Adapter<TrxAdapter.TrxViewHolder>(){
    ...
    override fun onBindViewHolder(holder: TrxViewHolder, position: Int) {
       val trx = trxList[position]
        ...
        holder.itemView.setOnClickListener {
            if (code == "1"){
                val i = Intent(holder.itemView.context,DetailTrxActivity::class.java)
                i.putExtra("TRX_ID",trx.trxId)
                holder.itemView.context.startActivity(i)

            }else{
                val i = Intent(holder.itemView.context, DetailHisActivity::class.java)
                i.putExtra("TRX_ID",trx.trxId)
                holder.itemView.context.startActivity(i)
